...
TEST_CASE("PHDD is substituted", "[basic]")
{
    ...
    {
        CHECK(f.restr(0, true) == f_x0t);
        CHECK(f.restr(0, false) == f_x0f);
        CHECK(f.restr(1, true) == f_x1t);
        CHECK(f.restr(1, false) == f_x1f);
    }

    SECTION("Quantification")
    {
        CHECK(f.forall(0) == f_x0t * f_x0f);
        CHECK(f.forall(1) == f_x1t * f_x1f);
    }
}
